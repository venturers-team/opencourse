...
- 0점은 표의 기준이 문자 그대로 성립할 때만 준다. 문장이 스스로 다음 내용을 예고하거나 목록·질문을 시작하는 것은 "갑자기 튀는 것"(flow 0)이 아니다 — 빠진 단계가 실제로 있을 때만 0이다. "근거 없는 단정"(logic 0)은 학습자 상태와 어긋나거나 반박 가능한 주장일 때다 — 일상 경험으로 자명한 서술은 해당하지 않는다. 목록 항목·점검표 항목은 도입 문장 아래 이어지는 것으로 본다.
...
- add_facts에는 다음 문장을 읽는 데 필요한 담화 맥락도 담아라 — 방금 무엇이 예고·나열·질문되었는지 (예: "준비물 세 가지(노트북·인터넷·시간)가 나열되는 중이다", "버튼이 반응하지 않는 문제가 제기되었다").
